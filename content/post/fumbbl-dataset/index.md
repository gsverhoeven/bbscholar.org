...
This blogpost is about **Blood Bowl**, a boardgame of fantasy football that can also be played online at [FUMBBL.com](https://fumbbl.com). 
The goal of this blog post is to use Python API and HTML scraping to fetch online Blood Bowl match outcome data, and to create a publicly available dataset ready for analysis and visualization. 
In a [separate blog post](https://bbscholar.org/post/fumbbl-nufflytics/) I'll showcase some analyses on this dataset (also known as [Nufflytics](https://www.nufflytics.com) in reference to Nuffle, the god of Blood Bowl). The dataset is primarily constructed to analyze rule changes that were introduced on FUMBBL.com last year, and how these affected the relative strengths of the different teams coaches can pick to field against each other.
